import collections, glob, hashlib, io, json, os, platform, pytz, re, requests, socket, string, struct, time, unicodedata, xbmc, xbmcaddon
from contextlib import closing

from resources.lib.base.l1.constants import ADDON_ID, ADDON_PATH, ADDON_PROFILE, CONST_EPG, CONST_IMAGES, CONST_MD5, CONST_MINIMALEPG, CONST_RADIO, CONST_SETTINGS, DEFAULT_USER_AGENT, EPG_DB_FILE, SETTINGS_DB_FILE
from resources.lib.base.l1.encrypt import Credentials
from resources.lib.base.l2 import settings
from resources.lib.base.l2.log import log
from resources.lib.constants import SETUP_DB_QUERIES
...
def clear_prefs_tests():
    ...
    for row in data:
        channels_ar.append(row['id'])

    channels = "', '".join(map(str, channels_ar))

    query = "DELETE FROM `prefs_{profile_id}` WHERE NOT id IN ('{channels}')".format(profile_id=1, channels=channels)
    query_settings(query=query, return_result=False, return_insert=False, commit=True)

    query = "DELETE FROM `tests_{profile_id}` WHERE NOT id IN ('{channels}')".format(profile_id=1, channels=channels)
    query_settings(query=query, return_result=False, return_insert=False, commit=True)
...
def create_playlist():
    ...
    for row in channels:
        ...
        if len(id) > 0:
            ...
            path = 'plugin://{addonid}/?_=play_video&channel={channel}&id={asset}&type=channel&pvr=1'.format(addonid=ADDON_ID, channel=row['id'], asset=row['assetid'])
...
def download_files():
    ...
    if int(profile_settings['epgrun']) == 0 or int(profile_settings['epgruntime']) < (int(time.time()) - 300):
        try:
            ...
            if is_file_older_than_x_days(file=ADDON_PROFILE + "epg_all.xml", days=1) or len(profile_settings['epg_md5']) == 0 or profile_settings['epg_md5'] != md5JSON[key]:
                download_epg()
        except:
            download_epg()
...
def mod_epg():
    if os.path.isfile(ADDON_PROFILE + "epg_all.xml"):
        prefs = load_prefs(profile_id=1)
        ...
        xml = load_file(file='epg_all.xml', isJSON=False)

        new_xml_start = '<?xml version="1.0" encoding="utf-8" ?><tv generator-info-name="{addonid}">'.format(addonid=ADDON_ID)
        new_xml_end = '</tv>'

        new_xml = new_xml_start

        result = re.findall(r'<channel(?:(?!<channel)(?!</channel>)[\S\s])+</channel>', xml)
        id_regex = r"id=\"([\s\S][^\"]+)\""

        for match in result:
            match2 = re.search(id_regex, match)

            if match2:
                id = unicode(match2.group(1))

                if check_key(channel_ar, id) and channel_ar[id] == 1:
                    new_xml += match
                    channel_ar2.append(id)

        for id in channel_ar2:
            result = re.findall(r'<!-- BEGIN {id} -->(?:(?!<!-- BEGIN)(?!<!-- END {id} -->)[\S\s])+<!-- END {id} -->'.format(id=id), xml)

            for match in result:
                new_xml += match

        if not new_xml == new_xml_start:
            write_file(file='epg.xml', data=new_xml +  new_xml_end, isJSON=False)
        else:
            write_file(file='epg.xml', data=xml, isJSON=False)
...
def update_prefs():
    ...
    for row in data:
        ...
        for key in keys:
            if not prefs or not check_key(prefs, id):
